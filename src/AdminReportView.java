...
public class AdminReportView extends View {
	...
	public AdminReportView() {
		...
		// //////////////////
		// create table
		AdminReportView.reprot = new JTable(new DefaultTableModel(TABLE_COLUMNS, 0));
		AdminReportView.reprot.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);

		// set different columns width
		TableColumnModel propertyTableColumnModel = AdminReportView.reprot
				.getColumnModel();
	...
	public void clearReportTable() {
		int numberOfRow = AdminReportView.reprot.getModel().getRowCount();
		if (numberOfRow > 0) {
			DefaultTableModel tableModel = (DefaultTableModel) AdminReportView.reprot
					.getModel();
